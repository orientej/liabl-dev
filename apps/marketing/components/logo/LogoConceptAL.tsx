// Concept A · L Variant
// Same document/seal architecture as Concept A, but the inner seal contains
// a stylized L letterform instead of a checkmark. This ties the brand name
// directly into the mark while keeping the document-and-seal narrative.
interface Props {
  size?: 'sm' | 'md' | 'lg'
  dark?: boolean
  markOnly?: boolean
...
export default function LogoConceptAL({ size='md', dark=false, markOnly=false }: Props) {
  const s = sizes[size]
  ...
  const Mark = (
    <svg width={s.mark} height={s.mark} viewBox="0 0 48 48" fill="none">
      {/* Outer document shape with corner fold — same as Concept A */}
      <path
      ...
      {/* Stylized L inside the seal — replaces the checkmark */}
      {/* L is built from two strokes: vertical spine + horizontal base */}
      <path
        d="M22 21 L22 30 L29 30"
        stroke="white"
        strokeWidth="2.6"
        strokeLinecap="round"
